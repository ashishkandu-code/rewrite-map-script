            ...
            while True:
                if len(formatted_from_to_strings) == 1:
                    ...
                    if action.lower() in positives:
                        ...
                        continue
                if first_message:
                    print("\nPlease select a domain from above to copy formatted links...")
                print("[!] Enter 0 to continue post checks")
                if first_message:
                    choice = input("\nEnter a domain # to copy: ")
                else:
                    choice = input("\nEnter another domain # to copy: ")
                first_message = False
                if not choice.isnumeric():
                    print("Please enter numbers only from above options!\n")
                    continue
                choice = int(choice)
                if choice == 0:
                    break
                try:
                    ...
                    logger.error("Please enter a number from above domains options!")
